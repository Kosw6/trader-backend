...
public class SecurityService {
    ...
    public boolean canAccessNote(Authentication authentication, Long noteId) {
        ...
        Note note = noteRepository.findById(noteId)
                .orElseThrow(() -> new BaseException(BaseResponseStatus.ACCESS_DENIED,"해당 노트의 접근 권한이 없습니다."));
        if (!note.getUser().getId().equals(((UserContext) authentication.getPrincipal()).getUserDto().getId())) {
            throw new BaseException(BaseResponseStatus.ACCESS_DENIED,"해당 노트의 접근 권한이 없습니다.");
        }
