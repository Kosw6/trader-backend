...
**시행일자:** 2025-10-19  
**테스트 목적:** 배포 서버 환경에서 k6 부하 안정임계값 테스트
...
2. Spring
   ...
   - tomcat.threads.min-spare: 30
   - tomcat.threads.connection-timeout: 10s
...
- [서버 부하테스트 세팅]() 해당 보고서에서 데이터베이스 힙용량과 부가적인 세팅을 변경하여 테스트시에 생기는 무의미한 부하를 줄였음
- 애플리케이션 PageController에서 제공하는 조회API를 테스트 하기 위해 안정임계값으로 rate를 올려가며 테스트 중에 과부하를 확인
