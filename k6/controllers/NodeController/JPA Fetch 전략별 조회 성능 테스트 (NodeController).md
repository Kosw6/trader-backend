...
- ①은 행 폭증으로 네트워크/매핑 오버헤드는 있지만, Hibernate가 부모 엔티티를 Identity Map으로 재사용(deduplicate) 하기 때문에, **부모 객체/대용량 필드 생성**이 row 수만큼 반복되지 않았다. 반면 JSON agg는 DB에서 그룹별 JSON 생성 및 정렬 비용이 발생해 DB CPU가 p95를 지배했다
